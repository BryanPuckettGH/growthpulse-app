...
- Marketing site caught up with the product. Everything shipped since v2.19.0 was invisible to a visitor: the site still sold a monitoring-only device with "4 sensors per node" and no mention anywhere of irrigation, the flow meter, water cost, leak detection, Google Home, or the iOS app. New "Watering" page covering timed runs, live flow confirmation, the self-closing dry-run cutoff, the leak alarm, meter-style cost accounting, and rain delay. The hero, the stats row, the home feature cards, the Features grid, and the FAQ all now describe a system that acts on what it measures instead of only reporting it.
- Footer gains a fourth column linking the project page, the team lead's note, and the GitHub repository, and a Watering link under Product.
- `.proj__numbers` grid children carry `min-width: 0` so the stats row cannot force horizontal overflow on a narrow window.
