# GrowthPulse User Manual

Smart Plant Monitoring · growthpulsecloud.com
...
**The app lives at growthpulsecloud.com.** It works in any browser on your phone, tablet, or computer, and it can be installed on your phone's home screen like a regular app (see "Install the App on Your Phone").
...
![Your GrowthPulse device](assets/Device Overview.svg)
...
- **Pairing page**: your unit's pairing code in large letters, with a status indicator and the firmware version. **ON** means your unit is connected and streaming; two dots (..) mean it is still connecting.
...
The screen dims itself after a few idle minutes to stay fresh; tap the PRG button once to wake it.

**The PRG button.** One button does everything, and the screen always tells you which action you are about to get while you hold it:
...
- **Hold 10 seconds**: full reset. The unit forgets its saved Wi-Fi and reopens setup mode. Use this when you move, change your Wi-Fi password, or hand the unit to a new owner. The screen tells you when the reset is about to happen.
...
3. If your phone says "this network has no internet access," choose **Keep trying Wi-Fi** or **Use this network anyway**. That is expected; the unit is not on the internet yet, which is exactly why you are setting it up.
...
**Forgot your password?** Use the link on the sign-in screen; we email you a secure link that opens a set-new-password page.
...
**The moisture gauge and health score.** The dial shows soil moisture at a glance. Next to it, the **plant health score** (0 to 100) blends every connected sensor against your plant's ideal ranges. Below them you can see how the device is connected (with its real signal strength), its power status, and the plant's location.

**Plant profile.** Tap the plant bar to tell GrowthPulse what you are growing: basil, tomatoes, succulents, ferns, orchids, and more than 140 others, searchable by name or scientific name. Every plant type has its own ideal ranges, so the colors, health score, and advice are tuned to that plant, not a generic average.
...
**Automated watering.** With a Pro plan and a pump accessory, GrowthPulse can water automatically when soil gets too dry: pick manual, auto, or schedule mode, set the target moisture and pump duration, or tap Water now.

**Rain delay.** On Pro, turn on **rain delay** so automatic watering is skipped when rain is in the local forecast; no overwatering right before a storm. Because rain delay has to look up the forecast where the plant lives, it needs that plant's home location. If the plant has no location yet, the rain delay switch explains why and lets you add one on the spot. This is the one feature that requires a location, and it tells you exactly which plant still needs one.
...
The Devices tab lists every plant on your account, each with its photo, pairing code, connection and signal, power status, and current moisture.

**Photos.** Give any plant a profile photo: take one or upload one, then crop it square. The photo shows on the device card, the dashboard, and follows your account everywhere.
...
**Power status.** Each card shows how the unit is powered: **AC power** for plugged-in units, or a battery percentage with a charging indicator. Low battery turns the badge amber, then red, and the app warns you before the unit goes offline.

**Editing a device.** Tap the pencil on any card to rename the plant, set or change its home location, change its group or photo, or switch how it connects (Wi-Fi or LoRaWAN gateway).

**Switching how a node connects.** In the edit screen you can move a node between Wi-Fi and LoRaWAN, and the app does the whole setup for you. Switching a Wi-Fi node to LoRaWAN sets it up on your gateway behind the scenes; the node reboots and joins on its own, no codes to copy anywhere. Switching a LoRaWAN node back to Wi-Fi sends it a message that takes effect on its next check-in (LoRaWAN nodes check in every few minutes, so give it a minute or two), after which it rejoins your Wi-Fi. The plant stays the same plant on your dashboard the whole time; only the way it reaches the internet changes.
...
**Selling or gifting a unit: Factory reset.** Also in the danger zone. This permanently deletes everything tied to the plant from your account, and if the unit is online, it remotely wipes its own Wi-Fi and reboots into setup mode, ready for its new owner to unbox a fresh experience. Read the confirmation carefully: this cannot be undone. (If the unit is offline at the time, the new owner can simply hold PRG for 10 seconds to reset it.)

---

# Power, Battery, and Sleep

Most units run on USB-C wall power and show **AC power** on their card. Battery-powered units (and Farm Kit nodes) show a **battery percentage** with a charging indicator, and the card turns amber, then red, as the battery runs low, with a warning in the app before the unit goes offline.
...
- **Add the gateway in the app**: Devices tab, **Add a LoRaWAN gateway**, then enter or scan the Gateway EUI from its label (the 16-character ID on the EUI line; the camera scanner reads the label's QR code for you). The app registers the gateway on the network for you. You never log in to any network console.
...
| Plants | Up to 3 | Up to 10 | Unlimited |
| Live dashboard, health score, plant profiles | Yes | Yes | Yes |
| History charts and alarms | Yes | Yes | Yes |
| Weather rain gauge, rain and sun alerts | | Yes | Yes |
| Automated watering with rain delay | | | Yes |
| LoRaWAN gateways (Farm Kit) | | | Yes |

See your current plan and upgrade any time in Settings.
...
- **Refresh rate**: how often the app updates readings, from every second to every 5 minutes. Second-by-second rates apply to Wi-Fi devices; LoRaWAN nodes check in every few minutes by design. On battery-powered units, faster updates mean shorter battery life.
...
**Where to find it.** Open **Settings** and tap **Download report (PDF)**.
...
- **Sensors**: include or leave out Temperature, Humidity, Soil Moisture, and Soil Temp.
...
A loading screen with a spinner shows while the report builds (it can take a few seconds for long periods), so you know it is working; no need to tap again or refresh.
...
1. Open **growthpulsecloud.com** in Safari (iPhone) or Chrome (Android).
2. Tap the **Share** button, then **Add to Home Screen** (Android: menu, "Add to home screen").
3. You get a GrowthPulse icon on your home screen, and the app opens full screen, just like an app from the App Store.
...
| Download PDF seems slow the first time | The high-quality renderer wakes up on first use, then is quick. The spinner means it is working; it will save automatically. |
...
| The screen is dark and the unit seems off | It is probably asleep (a double-tap of PRG puts it to sleep). Tap PRG once or press RESET to wake it. |
| The gateway scanner camera does not start | Allow camera access for growthpulsecloud.com when your browser asks, then tap the scan button again. You can always type the EUI instead. |
| I switched a node to Wi-Fi but it is still on LoRaWAN | LoRaWAN nodes act on changes at their next check-in, every minute or two. Give it a couple of minutes; it switches and rejoins on its own. |
| A battery unit's percentage seems stuck right after plugging in | The percentage rises gently on purpose so it tracks the real charge instead of jumping. It keeps climbing while charging. |
...
**Does the app track my location?** No, and it never asks your browser or phone for it. Weather uses only the home location you choose to type for a plant (a city or ZIP). If you skip it, weather and rain delay are simply off for that plant and nothing else changes.

**Can I sign in on more than one device?** Yes. Your plants live with your account in the cloud, so signing in on a new phone or computer shows everything right away.
...
**What is the code on the screen for?** It is your unit's pairing code and serial number in one. You use it once to claim the unit, and support can use it to identify your exact device.
...
| Button | Single tap wake, double tap sleep, 3-second hold return-to-Wi-Fi, 10-second hold reset |
| Display | Built-in OLED, rotating pairing code, connection, and live readings pages |
